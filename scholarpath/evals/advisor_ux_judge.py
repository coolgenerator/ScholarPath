"""LLM-as-judge utilities for Advisor UX gold evaluation (A/B comparison)."""
...
class AdvisorUXJudgeCaseResult:
    case_id: str
...
@dataclass(slots=True)
class AdvisorUXJudgeRunSummary:
    status: str
...
class AdvisorUXABJudge:
    """A/B judge for advisor user-feel eval with structured, non-CoT outputs."""
        ...
        *,
        ...
        schema = {
            ...
            "properties": {
                ...
                "scores_option_1": {
                    "type": "object",
                    "properties": {dim: {"type": "number"} for dim in RUBRIC_DIMENSIONS},
        ...
        prompt = {
            "task": (
                "Compare two assistant outputs for user experience quality. "
                "Use the 7-dimension rubric only. Score each dimension in [0,5]. "
        ...
        try:
            judged = await self._llm.complete_json(
                [
                    {
                        "role": "system",
                        "content": (
                            "You are a strict evaluator for Advisor UX. "
                            "Do not reveal reasoning chains. Return only schema-compliant JSON."
                        ),
                    },
                ...
                caller=f"eval.advisor.ux.judge.case#{run_id}",
            )
        except Exception as exc:
            logger.warning("Advisor UX judge case failed: %s", exc)
            zero = {dim: 0.0 for dim in RUBRIC_DIMENSIONS}
            return AdvisorUXJudgeCaseResult(
                case_id=case_id,
        ...
        if a_is_candidate:
            ...
            winner = self._map_winner_option(str(judged.get("winner_option") or "tie"), "option_1")
        else:
            candidate_scores = scores_2
            baseline_scores = scores_1
            winner = self._map_winner_option(str(judged.get("winner_option") or "tie"), "option_2")
    ...
    async def judge_run(
        ...
        *,
        run_id: str,
        case_results: list[AdvisorUXJudgeCaseResult],
        ...
        if not scored_cases:
            return AdvisorUXJudgeRunSummary(
                status="partial",
                candidate_win_rate=0.0,
                mean_delta_by_dim={dim: 0.0 for dim in RUBRIC_DIMENSIONS},
        ...
        overall_user_feel_mean = round(
            self._mean(item.candidate_mean for item in scored_cases if item.candidate_mean is not None),
            4,
        )

        prompt = {
            "task": (
                "Summarize A/B UX judge outcomes with concise risks and recommendations. "
        ...
        try:
            judged = await self._llm.complete_json(
                [
                    {
                        "role": "system",
                        "content": (
                            "You summarize eval outcomes for engineers. "
                ...
                caller=f"eval.advisor.ux.judge.run#{run_id}",
            )
            ...
            logger.warning("Advisor UX judge run failed: %s", exc)
            return AdvisorUXJudgeRunSummary(
                status="partial",
                candidate_win_rate=round(candidate_win_rate, 4),
                mean_delta_by_dim=mean_delta_by_dim,
    ...
    def _shuffle_pair(
        *,
        ...
        rng = random.Random(f"{run_id}:{case_id}")
        if rng.random() < 0.5:
    ...
    def _normalize_scores(raw: Any) -> dict[str, float]:
        ...
        out: dict[str, float] = {}
        for dim in RUBRIC_DIMENSIONS:
            out[dim] = AdvisorUXABJudge._clamp(float(raw.get(dim) or 0.0), 0.0, 5.0)
        return out

    @staticmethod
    def _map_winner_option(winner_option: str, candidate_option: str) -> str:
        option = winner_option.strip().lower()
        if option == "tie":
            return "tie"
        if option == candidate_option:
            return "candidate"
        if option in {"option_1", "option_2"}:
            return "baseline"
        return "tie"
...
def create_unscored_case_result(
    *,
    case_id: str,
    reason: str,
    notes: str = "",
